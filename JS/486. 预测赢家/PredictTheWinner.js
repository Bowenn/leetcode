...
const PredictTheWinner1 = function (nums) {
    ...
    const a = getMax(0, nums.length - 1);
    return a >= sum / 2;
};

/* My Solution Base 未优化 316 ms */
const PredictTheWinner0 = function (nums) {
    const sum = nums.reduce((s, cur) => s + cur);
    const getMax = (begin, end, sum_sub) => {
        if (begin === end) {
            return nums[begin];
        } else {
    ...
    const a = getMax(0, nums.length - 1, sum);
    return a >= sum / 2;
};
